...
exports = module.exports = async function newProject(project) {
  ...
  spinner.start();
  spinner.text = chalkInfo(`正在拉取模板代码，下载位置：${pwd}/${project}/ ...`);

  try {

    verbose(await execPromise(`git clone ${boilerplateRepo} ./${project} --depth 1`, { cwd: pwd }))

    let newDropsBuffer = await fsExtra.readFile(path.resolve(pwd, project, 'boilerplate-new'))

    // 读取文件，过滤出要删除的文件或文件夹
    let files = filterFiles(newDropsBuffer.toString())
    if (files.length) {
      spinner.text = '正在删除不必要的文件'
      await Promise.all(files.map(file => fsExtra.remove(path.resolve(pwd, project, file))))
    }
